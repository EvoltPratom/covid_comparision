...
def get_cases(c_name, type_of_data):
    ...
    # values = y[7].split('data:')[1].split('}],')[0][2:].split(']')[0].split(",")
    # to_ret = dict(zip(dates, values))
    # return to_ret

    dates = y[y.find('categories:['):y.find(']')]
    dates = dates[13:].replace('"', '').split(',')

    values = y[y.find('data:['):y.find(']},{name')]
    values = values[6:].replace('null', str(0)).split(',')
